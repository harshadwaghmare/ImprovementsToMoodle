...
if ($id) {
    ...
    $PAGE->set_url(new moodle_url('/course/smanage.php', array('categoryid' => $id)));
    ...
    $PAGE->set_url(new moodle_url('/course/smanage.php'));
...
if (!empty($deletecat) and confirm_sesskey()) {
    ...
    if ($mform->is_cancelled()) {
        redirect(new moodle_url('/course/smanage.php'));
    }
    ...
    if ($data = $mform->get_data()) {
        ...
        if ($data->fulldelete == 1 && $cattodelete->can_delete_full()) {
            ...
            echo $OUTPUT->continue_button(new moodle_url('/course/smanage.php'));
...
if (can_edit_in_category()) {
    ...
    admin_externalpage_setup('coursemgmt', '', $urlparams, $CFG->wwwroot . '/course/smanage.php');
...
if (!empty($searchcriteria)) {
    ...
    $table->head[] = get_string('move');
    $table->head[] = get_string('delete');
    $table->head[] = get_string('hide/show');
    ...
    $url = new moodle_url('actioncat.php');
    echo html_writer::start_tag('form', array('action' => $url, 'method' => 'post'));
    echo html_writer::start_tag('div', array('align' => 'right'));
    echo html_writer::table($table);
    echo html_writer::empty_tag('input', array('type' => 'hidden', 'name' => 'sesskey', 'value' => sesskey()));
    echo html_writer::empty_tag('input', array('type' => 'submit', 'name' => 'submit', 'value' => 'submit', 'style' => 'width: 150px; height: 30px;'));
    ...
    $select = new single_select(new moodle_url('/course/smanage.php'), 'categoryid', $displaylist, $coursecat->id, null, 'switchcategory');
...
if (!empty($searchcriteria)) {
    ...
    $table->data = array();
    $baseurl = new moodle_url('/course/smanage.php');
...
if (!$courses) {
    ...
    $baseurl = new moodle_url('/course/smanage.php', $urlparams + array('sesskey' => sesskey()));
...
/*        $PAGE->requires->yui_module('moodle-core-formautosubmit',
    ...
    $actionurl = new moodle_url('/course/smanage.php');
    $pagingurl = new moodle_url('/course/smanage.php', array('categoryid' => $id, 'perpage' => $perpage) + $searchcriteria);
...
if ($canmanage and $numcourses > 1 && empty($searchcriteria)) {
    ...
    $url = new moodle_url('/course/smanage.php', array('categoryid' => $id, 'resort' => 'name', 'sesskey' => sesskey()));
...
$id = optional_param('categoryid', 0, PARAM_INT);
if ($id !== 0) {
    $rurl = new moodle_url('staticoperations.php?categoryid='.$id);
    echo $OUTPUT->single_button($rurl, get_string('staticops'));
}
//end harshad
...
function print_category_edit(html_table $table, coursecat $category, $depth = -1, $up = false, $down = false, $displaymovecategoryto = true) {
    ...
    if ($category->id) {
        ...
        $categoryurl = new moodle_url('/course/smanage.php', array('categoryid' => $category->id, 'sesskey' => sesskey()));
        ...
        if (has_capability('moodle/category:manage', $categorycontext)) {
            ...
            // Delete category.
            $icons[] = $OUTPUT->action_icon(
                new moodle_url('/course/smanage.php', array('deletecat' => $category->id, 'sesskey' => sesskey())),
            ...
            if (!empty($category->visible)) {
                $icons[] = $OUTPUT->action_icon(
                    new moodle_url('/course/smanage.php', array('hidecat' => $category->id, 'sesskey' => sesskey())),
                    new pix_icon('t/hide', $str->hide, 'moodle', array('class' => 'iconsmall')),
                    null, array('title' => $str->hide)
                );
            } else {
                $icons[] = $OUTPUT->action_icon(
                    new moodle_url('/course/smanage.php', array('showcat' => $category->id, 'sesskey' => sesskey())),
            ...
            if ($up) {
                $icons[] = $OUTPUT->action_icon(
                    new moodle_url('/course/smanage.php', array('moveupcat' => $category->id, 'sesskey' => sesskey())),
            ...
            if ($down) {
                $icons[] = $OUTPUT->action_icon(
                    new moodle_url('/course/smanage.php', array('movedowncat' => $category->id, 'sesskey' => sesskey())),
        ...
        if ($displaymovecategoryto && has_capability('moodle/category:manage', $categorycontext)) {
            $id = $category->id;
            $movetocategories = array(0 => get_string('top')) + coursecat::make_categories_list('moodle/category:manage', $category->id);
            $text = html_writer::label(get_string('frontpagecategorynames'), 'movetocat', false, array('class' => 'accesshide'));
            $text .= html_writer::select($movetocategories, 'movetocat'.$category->id, $category->parent, null, array('id' => 'movetoid', 'class' => 'submit', 'style' => 'width: 180px; height: 30px;', 'formid' => 'moveform$category->id'));
            $actions = $text;
        }
        ...
        $thisrowdata[] = new html_table_cell(html_writer::empty_tag('input', array('type' => 'checkbox', 'name' => 'cmove'.$category->id, 'value' => 1)));
        $thisrowdata[] = new html_table_cell(html_writer::empty_tag('input', array('type' => 'checkbox', 'name' => 'cdel'.$category->id, 'value' => 1)));
        $thisrowdata[] = new html_table_cell(html_writer::empty_tag('input', array('type' => 'checkbox', 'name' => 'chid'.$category->id, 'value' => 1)));
